...
This repository is the source of [docs.apowerb.com](https://docs.apowerb.com). Pages are
MDX; navigation and theming live in `docs.json`.
...
**A merge does not publish.** The hosting platform does not build on push: the live site
changes only when someone triggers *Manual update* from its dashboard. Merging your PR
...
keeps being served, so the reference drifts silently. Diff the `paths` to check
