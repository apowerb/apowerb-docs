...
A chart can take `source_type: "agent"`, in which case its rows are whatever the agent
...
| Field | Description |
| --- | --- |
| `agent_ids` | The agents to run. Their rows are concatenated in order. |
| `message` | What to ask them. Required in practice — see below. |
...
```json
{
  ...
  "source_options": {
    "agent_ids": [7],
    "message": "Monthly revenue by region for the last 12 months"
  }
}
```

Without a `message`, the agent is only asked for the latest data with no indication of
...
Refreshes run through the [orchestrator](/guides/orchestrator), so they obey the same
